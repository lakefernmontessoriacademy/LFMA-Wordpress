...
if ( class_exists( 'LP_Abstract_Question' ) ) {
	...
	class LP_Question_Fill_In_Blank extends LP_Abstract_Question {
		...
		function shortcode( $atts = null ) {
			...
			return sprintf(
				'<input type="text" name="%s" data-fill="%s" value="%s" %s />%s',
				'learn-press-question-' . $this->id . '[' . $input_name . '][]',
				esc_attr( $input_name ),
				$atts['user_fill'],
				$atts['correct_fill'] ? ' disabled="disabled"' : '',
				$atts['correct_fill'] ? sprintf( '<span class="check-label %s">%s</span>', $atts['user_fill'] == $atts['correct_fill'] ? 'correct' : 'wrong', $atts['correct_fill'] ) : ''
			);
		}
		...
		function render( $args = null ) {
			...
			$view = LP_Addon_Question_Fill_In_Blank::locate_template( 'fill-in-blank.php' );
			include $view;
